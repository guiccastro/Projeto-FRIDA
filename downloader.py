...
# The downloaded files will be in the path that user pass as the parameter, if the path is passed, than the default 
# path is downloader.py-PATH/Data. Some error logs can be generated by the code. Each project will have a separate 
# directory to keep its logs from the download errors. The directories are: downloader.py-PATH/Downloader Logs/PROJECT NAME. 
# The logs will have name in the pattern: Log-DATE, the date is to tell the user which date file could not be downloaded.


##############################################################################
# TO DO AND IDEAS:
# [ ] - Instead of creating the list with the file names that must be downloaded, an idea is to use an API that list the possible files from each url (I don't know if this kind of API exists)
#       and verifies by the date which file in the list must be downloaded.
# [ ] - Create the function to download files based on the frequency of days and months.


##############################################################################
# KNOWN ISSUES (SOLUTIONS):
# Wget has a problem to download from Isolario, an SSL configuration was used to Wget don't do some verifications when download.
...
### PATH TO SAVE FILE ### (If not passed, the default path is "Data/". The path doesn't need to exist, because the code will create the directory if not, but the path must be in the same directory as this code, otherwise wget will not download the file)
# -S:path
    ...
    if(parameter[0] != "-"):
        ...
        # Begin date and time
            ...
            if(len(parameter) > 2 and parameter[2] == ":" and parameter[3:].count(":") == 1 and parameter[3:].count("/") == 2 and parameter[3:].count(",") == 1):
                ...
                # Verifies if the date is write correctly
                ...
                # Verifies if the time is write correctly
        ...
        # Frequency
            ...
            # Verifies which type of frequency and if the frequency is write correctly
        ...
        # Download from Isolario
            ...
            if(len(parameter) > 2 and parameter[2] == ":"):
                ...
                # Verifies if the collector are in the list of collectors (to see if they are write correctly and if the collector exists)
...
if(not(error)):
    ...
    if(download_from_RouteViews):
        ...
        for collector in list(chosen_collectors_RouteViews):
            ...
            # Verifies which collecter was chosen, beacause depending on the collector, the final URL will be a different pattern
            # Create the URL's prefix with the chosen collector
    ...
    # Configuration to Wget don't have a problem to download from Isolario
    ...
    # Verifies if a directory for the logs of Isolario exists
    if(not path.exists("Downloader Logs/Isolario/")):

        # If not, create one
        os.mkdir("Downloader Logs/Isolario")

    # Verifies if a directory for the logs of RouteViews exists
    if(not path.exists("Downloader Logs/RouteViews/")):

        # If not, create one
        os.mkdir("Downloader Logs/RouteViews")

    # Verifies if a directory for the logs of RIPE exists
    ...
    # Verifies if there is logs for Isolario
    ...
    # Verifies if there is logs for RouteViews
    ...
    # Verifies if there is logs for RIPE
